...
| Paragraph indent | **Two ways, one width (`proseIndent`, 4ch).** Tab types a tab on the writing surface (`TabKeyPlugin`; a list item nests instead, Shift+Tab still leaves), and "Indent first lines" in the surface's menu indents every left-aligned paragraph. Both reach print and the DOCX, which writes a real `w:tab` and a half-inch first line. The element, tome and author fields keep Tab for moving between fields. |
...
| 2026-09-12 | **Tab types a tab** on the writing surface, and an **optional first-line indent**; both carried into print and DOCX. | Authors who learned on a typewriter indent with Tab, and the browser was taking the key for focus. The automatic indent is the typeset alternative; sharing one width makes the two look alike. |
